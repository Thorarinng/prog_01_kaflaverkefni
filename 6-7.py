# Write a Python program that reads an integer from the user 
# and prints out the binary equivalent number.

# How do we get a binary string from an integer? The easiest 
# method uses integer division by 2 on successive quotients and 
# then collects the remainders. It is best illustrated by an example. 
# Consider the integer 156:
...
# Stop at reaching a quotient of 0. The binary equivalent 
# is given by concatenating the remainders, in reverse 
...
# print("{:b}".format(my_int))

bstr = "{:b}".format(my_int)
